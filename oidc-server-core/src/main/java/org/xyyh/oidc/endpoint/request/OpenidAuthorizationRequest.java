...
import static org.xyyh.oidc.collect.Maps.hashMap;
...
//        this.authorizationUri = authorizationUri;
    ...
    public static OpenidAuthorizationRequest of(String uri, MultiValueMap<String, String> parameters) {
        ...
        parameters.entrySet().stream()
            .filter(entry -> OpenidAuthorizationRequest.isNotAuthorizationRequestParam(entry.getKey()))
            .forEach(entry -> additionalParameters.put(entry.getKey(), entry.getValue()));
        Set<String> scopes = StringCollectionUtils.split(parameters.get(OAuth2ParameterNames.SCOPE));
        Set<String> responseTypes = StringCollectionUtils.split(parameters.get(OAuth2ParameterNames.RESPONSE_TYPE));
